    ...
    public class ImageManagementService : IImageManagementService
    {
        ...
        public async Task<IEnumerable<DockerImageDto>> ListImagesAsync(bool all = false)
        {
            try
            {
                ...
                return response.Select(i => new DockerImageDto // Map DockerImage to DockerImageDto
                {
                    Id = i.ID.Split(':').LastOrDefault()?.Split('@').First().Substring(0,
                             Math.Min(12, i.ID.Split(':').LastOrDefault()?.Split('@').First().Length ?? 0)) ??
                         i.ID.Substring(0, Math.Min(12, i.ID.Length)), // Truncate to 12 characters
                    FullId = i.ID,
        ...
        public async Task<(bool Success, string? ErrorMessage)> RemoveImageAsync(string imageId, bool force = false,
            bool pruneChildren = false)
        {
            ...
            try
            {
                var result = await _client.Images.DeleteImageAsync(imageId,
                    new ImageDeleteParameters
                        { Force = force, NoPrune = !pruneChildren }); // NoPrune=true means don't prune parents

                _logger.LogInformation(
                    "Image '{ImageId}' smazán. Výsledek: {DeleteResultCount} položek smazáno/uvolněno.", imageId,
                    result?.Count ?? 0);
            ...
            catch (DockerApiException ex) when (ex.StatusCode == System.Net.HttpStatusCode.Conflict)
            {
                _logger.LogWarning(ex,
                    "Konflikt při mazání image '{ImageId}' (pravděpodobně používán kontejnerem nebo má závislé potomky). Force={ForceFlag}",
                    imageId, force);

                return (false,
                    $"Konflikt: Image je používán nebo má závislé potomky. Zkuste nejprve smazat/zastavit kontejnery nebo použít 'force'. Chyba: {ex.Message}");
            }
            catch (Exception ex)
            {
