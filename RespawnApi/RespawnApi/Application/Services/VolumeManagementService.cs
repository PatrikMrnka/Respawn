// File: haha/RespawnApi/RespawnApi/Application/Services/VolumeManagementService.cs
using Docker.DotNet;
...
namespace RespawnApi.Application.Services
{
    ...
    public class VolumeManagementService : IVolumeManagementService
    {
        ...
        public async Task<bool> RemoveVolumeAsync(string volumeName, bool force = false)
        {
            try
            {
                _logger.LogInformation("Pokouším se smazat volume '{VolumeName}' s force={ForceFlag}.", volumeName, force);
            ...
            {
                _logger.LogWarning(ex, "Konflikt při mazání volume '{VolumeName}' (pravděpodobně používán). Force={ForceFlag}", volumeName, force);
                return false;
            }
            catch (Exception ex)
            {
