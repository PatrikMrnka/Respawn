...
namespace RespawnApi.Application.Services
{
    ...
    public class ContainerManagementService : IContainerManagementService
    {
        ...
        private readonly IVolumeManagementService _volumeManagementService; // for deleting volume
            ...
            IVolumeManagementService volumeManagementService)
        {
            ...
            var dockerApiUri =
                _configuration
                    ["DockerSettings:DockerApiUri"]; // connecting to Docker API - port forward  ssh -N -L localhost:23750:/var/run/docker.sock optimus(ssh client) for example
            ...
            _logger.LogInformation(
                "ContainerManagementService inicializován s URI: {DockerApiUri} a base image: {LgsmBaseImage}",
                dockerApiUri, _lgsmBaseImage);
        }
            ...
            GameServer serverDetails, string gameImageTag, string gameIdentifier, string? additionalGsParams)
        {
            ...
            var uniqueSuffix = serverDetails.GameServerId.ToString("N").Substring(0, 8);
            var containerName = $"lgsm-{gameIdentifier}-{uniqueSuffix}";
            var volumeName = $"lgsm-data-{gameIdentifier}-{uniqueSuffix}";

            _logger.LogInformation(
                "Pokus o vytvoření kontejneru: Name={ContainerName}, Image={ImageName}, Volume={VolumeName} pro GameServerId={GameServerId}",
            ...
            try
            {
                ...
                {
                    // for now Create volume from docker api
                    ...
                    _logger.LogInformation("Volume {VolumeName} vytvořen pro kontejner {ContainerName}.", volumeName,
                        containerName);
                }

                var envVars = new List<string> { "SKIP_UPDATE=TRUE" }; // skipping update on linuxgsm server
                if (!string.IsNullOrWhiteSpace(additionalGsParams))
                {
                    envVars.Add($"GS_PARAMS={additionalGsParams}");
                }

                var createParams = new CreateContainerParameters // creating container parameters
                {
                    Image = imageName,
                    Name = containerName,
                    Env = envVars,
                    Labels = new Dictionary<string, string>
                    {
                        ...
                        { "com.respawn.lgsm.volume", volumeName }
                    },
                ...
                _logger.LogInformation("Kontejner {ContainerName} (ID: {ContainerId}) vytvořen.", containerName,
                    response.ID);

                if (await StartContainerAsync(response.ID)) // try ty start the container
                {
        ...
        public async Task<bool> StartContainerAsync(string containerId)
        {
            try
            {
                ...
                if (result)
                {
                    _logger.LogInformation("Kontejner {ContainerId} úspěšně spuštěn.", containerId);
                }
                else
                {
                    _logger.LogWarning("Příkaz ke spuštění kontejneru {ContainerId} nevrátil úspěch (výsledek false).",
                        containerId);
                }
        ...
        public async Task<bool> StopContainerAsync(string containerId)
        {
            try
            {
                ...
                var result = await _client.Containers.StopContainerAsync(containerId,
                    new ContainerStopParameters { WaitBeforeKillSeconds = 15 });
                if (result)
                {
                    _logger.LogInformation("Kontejner {ContainerId} úspěšně zastaven.", containerId);
                }
                else
                {
                    _logger.LogWarning("Příkaz k zastavení kontejneru {ContainerId} nevrátil úspěch (výsledek false).",
                        containerId);
                }
        ...
        {
            _logger.LogInformation(
                "Požadavek na smazání kontejneru {ContainerId}. Smazat asociovaný volume: {RemoveVolumeFlag}",
            ...
            {
                _logger.LogWarning("Kontejner {ContainerId} nenalezen při inspekci před smazáním.", containerId);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Chyba při inspekci kontejneru {ContainerId} před smazáním.", containerId);
            }
            ...
            {
                _logger.LogWarning("Kontejner {ContainerId} nenalezen při pokusu o smazání (již byl smazán?).",
                    containerId);
                containerRemovedSuccessfully = true;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Chyba při mazání kontejneru {ContainerId}.", containerId);
            }

            if (removeAssociatedVolume &&
                inspect?.Config?.Labels?.TryGetValue("com.respawn.lgsm.volume", out var volumeName) == true &&
                !string.IsNullOrEmpty(volumeName)) // check if the label exists and is not empty
            {
                _logger.LogInformation(
                    "Identifikován asociovaný volume '{VolumeName}' pro kontejner {ContainerId} z labelu.", volumeName,
                    containerId);

                bool volumeRemoved =
                    await _volumeManagementService.RemoveVolumeAsync(volumeName,
                        true);

                if (volumeRemoved)
                {
                    _logger.LogInformation("Asociovaný volume '{VolumeName}' úspěšně smazán.", volumeName);
                }
                else
                {
                    _logger.LogWarning("Nepodařilo se smazat asociovaný volume '{VolumeName}'.", volumeName);
                }
            }
            else if (removeAssociatedVolume)
            {
                _logger.LogWarning(
                    "Nepodařilo se identifikovat asociovaný volume pro kontejner {ContainerId} z labelu 'com.respawn.lgsm.volume' nebo inspekce selhala.",
                    containerId);
            }
        ...
        public async Task<List<string>> GetContainerLogsAsync(string containerId, DateTime? since = null,
            uint lines = 200)
        {
            // Initialize a list to hold log lines
            ...
            try
            {
                ...
                using var logsStreamMultiplexed =
                    await _client.Containers.GetContainerLogsAsync(containerId, parameters, CancellationToken.None);
                ...
                {
                    var readResult =
                        await logsStreamMultiplexed.ReadAsync(buffer, 0, buffer.Length, CancellationToken.None);
                ...
                _logger.LogInformation("Načteno {Count} řádků logů pro kontejner {ContainerId}.", logLines.Count,
                    containerId);
            }
            catch (DockerContainerNotFoundException)
            {
                logLines.Add("Kontejner nenalezen.");
            }
            catch (Exception ex)
            {
                logLines.Add($"Chyba při získávání logů: {ex.Message}");
            }

            return logLines;
        }

        public async Task StreamContainerLogsAsync(string containerId, Func<string, Task> onLogLineReceived,
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                logsStreamMultiplexed =
                    await _client.Containers.GetContainerLogsAsync(containerId, false, parameters, cancellationToken);
                ...
                {
                    var result =
                        await logsStreamMultiplexed.ReadOutputAsync(buffer, 0, buffer.Length, cancellationToken);
                ...
                await onLogLineReceived(
                    $"[SYSTEM] Streamování logů pro kontejner {containerId} bylo {(cancellationToken.IsCancellationRequested ? "zrušeno" : "ukončeno")}.");
            }
            ...
            {
                logsStreamMultiplexed?.Dispose();
                _logger.LogInformation("Streamování logů pro kontejner {ContainerId} bylo definitivně ukončeno.",
                    containerId);
            }
        }
        ...
        public async Task<IEnumerable<DockerContainerDto>> ListContainersAsync(bool all = true)
        {
            try
            {
                var response = await _client.Containers.ListContainersAsync(new ContainersListParameters { All = all }); // List all containers
                return response.Select(c => new DockerContainerDto // map response to DTO
                {
                    Id = c.ID,
                    ...
                    Command = c.Command,
                    Created = c.Created,
                    Ports = c.Ports?.Select(p => new DockerContainerPortDto
                            { PrivatePort = p.PrivatePort, PublicPort = p.PublicPort, Type = p.Type, IP = p.IP })
                        .ToList() ?? new List<DockerContainerPortDto>(),
                    State = c.State,
                    Status = c.Status,
            ...
            {
                _logger.LogError(ex, "Chyba při výpisu Docker kontejnerů.");
                return Enumerable.Empty<DockerContainerDto>();
            }
        }
    }
}