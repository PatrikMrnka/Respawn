...
const API_BASE_URL = 'http://localhost:5207'; // Your API base URL
...
class ServerLogSignalRService {
  ...
  public async startConnection(): Promise<boolean> {
    ...
    if (!token) {
      console.warn('ServerLogSignalRService: No auth token available. Connection will not be started.');
      Swal.fire({
        icon: 'error',
    ...
    this.connection.onclose(error => {
      ...
      if (this.currentWatchingGameServerId && this.onLogLineReceivedCallback) {
        this.onLogLineReceivedCallback(`[SYSTEM] Spojení se serverem logů bylo přerušeno. Probíhá pokus o znovupřipojení... Error: ${error?.message || 'Unknown error'}`);
      }
    });
    ...
    this.connection.onreconnecting(error => {
        ...
        if (this.currentWatchingGameServerId && this.onLogLineReceivedCallback) {
            this.onLogLineReceivedCallback(`[SYSTEM] Ztráta spojení se serverem logů, pokus o znovupřipojení...`);
        }
    });

    this.connection.onreconnected(connectionId => {
        console.log(`ServerLogSignalRService: Connection reconnected with new connectionId: ${connectionId}. Re-watching logs if previously watching.`);
        if (this.currentWatchingGameServerId && this.onLogLineReceivedCallback) {
  ...
  public async watchLogs(gameServerId: string, callback: (line: string) => void): Promise<void> {
    if (!this.connection || this.connection.state !== HubConnectionState.Connected) {
      console.warn('ServerLogSignalRService: Not connected. Attempting to start connection before watching logs.');
    ...
    try {
      ...
      console.log(`ServerLogSignalRService: Requested to watch logs for gameServerId: ${gameServerId}`);
    } catch (err) {
      console.error(`ServerLogSignalRService: Error invoking "WatchLogs" for ${gameServerId}:`, err);
      this.onLogLineReceivedCallback(`[SYSTEM ERROR] Nepodařilo se zahájit sledování logů pro server ${gameServerId}: ${err instanceof Error ? err.message : String(err)}`);
  ...
  public async unwatchLogs(gameServerId: string | null = null): Promise<void> {
    ...
    if (idToUnwatch && this.connection && this.connection.state === HubConnectionState.Connected) {
      try {
        ...
        console.log(`ServerLogSignalRService: Requested to unwatch logs for gameServerId: ${idToUnwatch}`);
      } catch (err) {
        console.error(`ServerLogSignalRService: Error invoking "UnwatchLogs" for ${idToUnwatch}:`, err);
