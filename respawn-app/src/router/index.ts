...
const routes: Array<RouteRecordRaw> = [
  ...
  // TODO: Přidat další routy pro /servers, /polls, /stats, /settings až budou implementovány
];
...
router.beforeEach((to, from, next) => {
  ...
  if (!authStore.token && localStorage.getItem('authToken')) {
    ...
    authStore.expiresAt = localStorage.getItem('authExpiresAt') ? new Date(localStorage.getItem('authExpiresAt')!) : null;
    authStore.checkTokenExpiration(); // Zkontroluje, zda token nevypršel
  }


  if (to.meta.requiresAuth && !authStore.isLoggedIn) {
    // Pokud routa vyžaduje přihlášení a uživatel není přihlášen,
    // přesměrujte na domovskou stránku nebo zobrazte login.
    // Prozatím přesměrujeme na home, ideálně by se zobrazil login modal.
    next({ name: 'home' }); 
    // Můžete zde také vyvolat login modal, pokud preferujete:
