...
<!-- prozatimni avatar jako url, bude přidáno načítání obrázku -->
...
const rules = {
  ...
  nicknameLength: (value: string) =>
    (value && value.length >= 3 && value.length <= 50) || 'Přezdívka musí mít 3-50 znaků.',
...
const onAvatarError = () => {
  if (editableProfile.avatarUrl !== defaultAvatar) {
    // Zabraňte smyčce, pokud i placeholder selže
...
onMounted(() => {
  if (authStore.isLoggedIn) {
    ...
    // Pokud uživatel není přihlášen, neměl by se na tuto stránku dostat
    loading.value = false
  }
})
</script>
