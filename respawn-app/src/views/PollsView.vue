  ...
  <v-container class="futuristic-page polls-view-container">
    ...
    <v-row justify="center" v-if="!loading && polls.length > 0">
      <v-col v-for="poll in sortedPolls" :key="poll.pollId" cols="12" md="10" lg="10">
        <v-card
          class="futuristic-card poll-card mb-6"
          :class="{ 'closed-poll': isPollEffectivelyClosed(poll) }"
        >
          ...
          <v-chip
            v-if="isPollEffectivelyClosed(poll)"
            color="grey"
            label
            small
            class="status-chip font-exo2"
            >Uzavřeno</v-chip
          >
          <v-chip
            v-else-if="userHasVoted(poll.pollId)"
            color="info"
            label
            small
            class="status-chip font-exo2"
          ...
          <v-card-subtitle class="pb-0 font-inter">
            Vytvořil: {{ poll.creatorNickname || poll.creatorUserId }} | Konec:
            <span :title="formatFullDateTime(poll.endTime)">{{
              formatRelativeTime(poll.endTime, currentTime)
            }}</span>
          ...
          <v-card-text class="pt-3">
            ...
            <div
              ...
              "
              class="font-inter text-grey-darken-1"
            >
              Tato anketa zatím nemá žádné možnosti hlasování.
            </div>

            <div
              v-if="isPollEffectivelyClosed(poll) || userHasVoted(poll.pollId)"
              class="results-section"
            >
              <p class="font-inter mb-2 text-h6">
                Výsledky (Celkem hlasů: {{ poll.totalVotes || 0 }}):
              </p>
              <div
                v-if="!poll.options || poll.options.length === 0"
                class="font-inter text-grey-darken-1"
              >
              ...
              <v-list v-else lines="one" dense class="results-list">
                ...
                >
                  <v-list-item-title class="font-inter d-flex align-center">
                    ...
                    {{ option.text }}
                    <v-icon
                      v-if="didUserVoteForOption(poll, option.optionId)"
                      color="primary"
                      size="small"
                      class="ml-2"
                  ...
                  <template v-slot:append>
                    <span class="font-roboto-mono"
                      >{{ option.voteCount }} ({{
                        calculatePercentage(poll.totalVotes || 0, option.voteCount || 0)
                      }}%)</span
                    >
                  </template>
                  <v-progress-linear
          ...
          <v-card-actions v-if="canManagePoll(poll)" class="poll-actions">
            ...
            <v-btn
              ...
              color="error"
              variant="text"
              @click="confirmDeletePoll(poll.pollId)"
              title="Smazat anketu"
            ></v-btn>
          </v-card-actions>
        </v-card>
...
const handleReceivePollUpdate = (updatedPollFromSignalR: Poll) => {
  ...
  if (index !== -1) {
    ...
    const mergedUserVotedOptionIds =
      ...
      updatedPollFromSignalR.userVotedOptionIds.length > 0
        ? updatedPollFromSignalR.userVotedOptionIds
        : existingPoll.userVotedOptionIds

    const isNowClosedBySignalR = !!(
      updatedPollFromSignalR.isClosed ||
      (updatedPollFromSignalR.endTime && new Date(updatedPollFromSignalR.endTime) <= new Date())
    )
    ...
    selectedOptions[updatedPollFromSignalR.pollId] = updatedPollFromSignalR.isMultipleChoice
      ...
      : updatedPollFromSignalR.userVotedOptionIds &&
          updatedPollFromSignalR.userVotedOptionIds.length > 0
        ? updatedPollFromSignalR.userVotedOptionIds[0]
        : ''
  }
}
...
const attemptPollsReconnect = async () => {
  ...
  if (
    signalRService.getConnectionState(POLL_HUB_PATH) === 'Disconnected' ||
    signalRService.getConnectionState(POLL_HUB_PATH) === null
  ) {
...
const formatRelativeTime = (isoDateTime: string, now: Date): string => {
  ...
  if (diffSeconds < -60) {
    ...
    if (diffDaysAbs > 1) return `${diffDaysAbs} dnů zpět`
    if (diffDaysAbs === 1) return `včera`
    if (diffHoursAbs > 1) return `${diffHoursAbs} hodin zpět`
    if (diffHoursAbs === 1) return `1 hodinu zpět`
    if (diffMinutesAbs > 1) return `${diffMinutesAbs} minut zpět`
    return 'před ${Math.abs(diffSeconds)} sekundami'
  }
  if (diffSeconds <= 0) return 'právě skončilo'
...
const canManagePoll = (poll: Poll): boolean => {
  ...
  return (
    poll.creatorUserId === authStore.user.id ||
...
const handleVote = async (pollId: string) => {
  ...
  if (optionIdsToSubmit.length === 0) {
    Swal.fire({
      ...
      text: 'Zvolte alespoň jednu možnost.',
      icon: 'error',
    })
    return
  }
...
const getPollOptionsWithResults = (poll: Poll): PollOption[] => {
  ...
  return poll.options
    .map((opt) => ({ ...opt, voteCount: opt.voteCount || 0 }))
...
const createIconHtml = (
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" role="img" aria-hidden="true" width="${size}" height="${size}" fill="white"" style="${extraStyle}"><path d="${pathData}"></path></svg>`
}
...
const openCreatePollModal = () => {
  ...
  Swal.fire({
    ...getFuturisticSwalBaseOptions('Nová anketa'),
    html: `
      <div id="createPollForm" class="swal-form-container">
        ...
        <input id="swal-question" class="swal2-input futuristic-swal-input" placeholder="Tvoje Otázka">
        <label for="swal-endTime" class="swal-label">
            ${createIconHtml(mdiCalendarClock, 18, iconColor, iconStyle)}Čas ukončení:
        </label>
        ...
        <label for="swal-imageUrl" class="swal-label">
            ${createIconHtml(mdiImage, 18, iconColor, iconStyle)}Obrázek URL (volitelné):
        </label>
        ...
        <label class="swal-label">
            ${createIconHtml(mdiFormatListBulletedSquare, 18, iconColor, iconStyle)}Odpovědi (min. 2):
        </label>
        ...
        <button id="swal-add-option" type="button" class="futuristic-btn-secondary">Přidat další</button>
        <label class="swal-checkbox-label">
          ...
          ${createIconHtml(mdiCheckboxMultipleBlankOutline, 18, iconColor, iconStyle)} Více možností odpovědí
        </label>
      </div>
    `,
    confirmButtonText: 'Vytvořit anketu',
    ...
    preConfirm: () => {
      ...
      const isMultipleChoice = (
        document.getElementById('swal-isMultipleChoice') as HTMLInputElement
      ).checked
      const optionsInputs = document.querySelectorAll(
        '#swal-options-container input[data-option-input-id]',
      )
      const options = Array.from(optionsInputs)
        .map((input) => ({ text: (input as HTMLInputElement).value.trim() }))
        .filter((opt) => opt.text !== '')

      let validationMessage = ''
      if (!question) validationMessage += 'Otázka je povinná!<br>'
      else if (question.length < 5) validationMessage += 'Otázka musí být delší než 5 znaků!<br>'
      if (!endTimeValue) validationMessage += 'Čas ukončení je povinný!<br>'
      else {
        const localEndDate = new Date(endTimeValue)
        if (localEndDate <= new Date())
          validationMessage += 'Čas ukončení musí být v budoucnosti!<br>'
      }
      if (options.length < 2) validationMessage += 'Musí existovat alespoň 2 možnosti!<br>'
      if (imageUrl && !/^https?:\/\/[^\s/$.?#].[^\s]*$/i.test(imageUrl))
        validationMessage += 'URL obrázku se nezdá být validní.<br>'
...
const openEditPollModal = (poll: Poll) => {
  ...
  let tempOptions: UpdatePollOptionPayload[] = JSON.parse(
    JSON.stringify(
      poll.options.map((o) => ({ optionId: o.optionId, text: o.text, imageUrl: o.imageUrl || '' })),
    ),
  )
  ...
  Swal.fire({
    ...
    html: `
      <div id="editPollForm" class="swal-form-container">
        ...
        ${
          !(poll.totalVotes && poll.totalVotes > 0)
            ? `<button id="swal-edit-add-option" type="button" class="futuristic-btn-secondary">Přidat možnost</button>`
            : ''
        }
        ${poll.totalVotes && poll.totalVotes > 0 ? '<p class="font-inter text-warning text-caption mt-2">Varování: Tato anketa již obsahuje hlasy. Změna možností není povolena.</p>' : ''}
      </div>
    `,
      ...
      const rebindRemoveListeners = () => {
        modalElement.querySelectorAll('.swal-remove-option-btn').forEach((btn) => {
          ...
          newBtn.addEventListener('click', (e) => {
            ...
            const indexToRemove = tempOptions.findIndex(
              (opt) =>
                (opt.optionId || `new-edit-option-${tempOptions.indexOf(opt)}-${Date.now()}`) ===
                tempIdToRemove,
            )
            if (indexToRemove !== -1) {
    ...
    preConfirm: () => {
      // Validate form inputs
      const question = (
        document.getElementById('swal-edit-question') as HTMLInputElement
      ).value.trim()
      const endTimeValue = (document.getElementById('swal-edit-endTime') as HTMLInputElement).value
      const imageUrl = (
        document.getElementById('swal-edit-imageUrl') as HTMLInputElement
      ).value.trim()
      ...
      if (!(poll.totalVotes && poll.totalVotes > 0)) {
        document
          .querySelectorAll('#swal-edit-options-container .swal-option-item')
          .forEach((itemDiv) => {
        ...
        poll.options.forEach((opt) =>
          finalOptions.push({ optionId: opt.optionId, text: opt.text, imageUrl: opt.imageUrl }),
        )
      }
      ...
      if (!question) validationMessage += 'Question is required.<br>'
        ...
        validationMessage += 'Question must be at least 5 characters long.<br>'
      if (!endTimeValue) validationMessage += 'End date and time is required.<br>'
        ...
        if (originalPollEndTime > new Date() && localEndDate <= new Date()) {
          validationMessage +=
            'New end time must be in the future if the poll has not ended yet.<br>'
        }
      }
      if (finalOptions.length < 2) validationMessage += 'At least two options are required.<br>'
      if (imageUrl && !/^https?:\/\/[^\s/$.?#].[^\s]*$/i.test(imageUrl))
        validationMessage += 'Image URL appears to be invalid.<br>'
...
const confirmDeletePoll = (pollId: string) => {
  Swal.fire({
    ...getFuturisticSwalBaseOptions('Opravdu chcete anketu smazat?'),
...
.winning-option .v-list-item-title,
.winning-option span {
